...
{
    double t;
    t = sqrt((double)pow((cir + number)->x - (cir + j)->x, 2.00) + (double)pow((cir + number)->y - (cir + j)->y, 2.00));
    if (t < (cir + number)->r + (cir + j)->r && (cir + number)->r < t + (cir + j)->r && (cir + j)->r < t + (cir + number)->r) {
        return 1;
    }
    return 0;
}